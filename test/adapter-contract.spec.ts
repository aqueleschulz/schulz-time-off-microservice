...
import { 
  InsufficientBalanceException, InvalidDimensionException, 
  DependencyUnavailableException, HcmContractViolationException, 
  CircuitBreakerOpenException, StaleBatchException
...
describe('Adapter and Contract Integrity Validation', () => {
  ...
  it('Translates HTTP Unprocessable Entity into Insufficient Balance Exception', async () => {
    httpMock.nextError = { response: { status: 422, data: { error: 'BALANCE_TOO_LOW', code: 'ERR_402' } } };
    await expect(adapter.deductBalance({} as never, 'deduct-key')).rejects.toThrow(InsufficientBalanceException);
  });

  it('Translates HTTP Not Found into Invalid Dimension Exception', async () => {
    httpMock.nextError = { response: { status: 404, data: { error: 'LOCATION_NOT_FOUND' } } };
    await expect(adapter.deductBalance({} as never, 'dim-key')).rejects.toThrow(InvalidDimensionException);
  });

  it('Translates Network Timeout into Dependency Unavailable Exception', async () => {
    httpMock.nextError = { code: 'ETIMEDOUT' };
    await expect(adapter.deductBalance({} as never, 'timeout-key')).rejects.toThrow(DependencyUnavailableException);
  });
  ...
  it('Validates Unexpected Response Schema Missing Required Fields', async () => {
    ...
    await expect(adapter.deductBalance({} as never, 'schema-key')).rejects.toThrow(HcmContractViolationException);
  });
  ...
  it('Activates Circuit Breaker After Consecutive Failures Limit', async () => {
    ...
    await expect(adapter.deductBalance({} as never, 'cb-key-6')).rejects.toThrow(CircuitBreakerOpenException);
  });

  it('Validates Batch Payload Schema and Rejects Missing Dimensions', async () => {
    const invalidBatchPayload = { batchId: 'batch-01', balances: [{ employeeId: 'E1' }] }; 
    await expect(adapter.processBatch(invalidBatchPayload as never)).rejects.toThrow(HcmContractViolationException);
  });
  ...
  it('Detects and Rejects Stale Batch Timestamps Older Than Local State', async () => {
    ...
    const staleBatchPayload = { batchId: 'batch-02', generatedAt: '2020-01-01T00:00:00Z', balances: [] };
    await expect(adapter.processBatch(staleBatchPayload as never)).rejects.toThrow(StaleBatchException);
  });

  it('Handles Negative Balance Returned by HCM as the Absolute Source of Truth', async () => {
    httpMock.nextResponse = { balance: -5.0, employeeId: 'E1', locationId: 'L1' };
  ...
  it('Throws Contract Violation Exception Upon Receiving Malformed JSON', async () => {
    ...
    await expect(adapter.getBalance('E1', 'L1')).rejects.toThrow(HcmContractViolationException);
  });
});