...
import { DependencyUnavailableException, InsufficientBalanceException } from '../src/domain/exceptions';
...
describe('TimeOffService - Critical Paths', () => {
  ...
  it('Partial HCM Success (500 Error After Deduction)', async () => {
    ...
    await expect(service.requestTimeOff(req, key)).rejects.toThrow(DependencyUnavailableException);
  ...
  it('Local Cache Stale (HCM Granted Bonus)', async () => {
    ...
    expect(mockRepo.getAuditLogs().some(l => l.type === 'JIT_HYDRATION')).toBeTruthy();
  });
  ...
  it('JIT Hydration Fails (HCM Timeout)', async () => {
    ...
    await expect(service.requestTimeOff(req, 'jit-fail')).rejects.toThrow(DependencyUnavailableException);
  ...
  it('JIT Hydration Reveals Negative Balance', async () => {
    ...
    await expect(service.requestTimeOff(req, 'jit-neg')).rejects.toThrow(InsufficientBalanceException);
