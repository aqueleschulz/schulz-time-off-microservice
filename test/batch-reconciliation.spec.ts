...
describe('Batch Reconciliation', () => {
  ...
  it('In-Flight Transaction During Batch', async () => {
    ...
    mockRepo.addAuditLog({ type: 'PENDING_HCM_ACK', amount: 3.0, createdAt: new Date('2026-04-24T14:05:00Z') });

    const batch = {
      batchId: 'b1', generatedAt: '2026-04-24T14:00:00Z',
  ...
  it('Batch with Invalid Data (Multi-Status)', async () => {
    const batch = {
      batchId: 'b2', generatedAt: new Date().toISOString(),
      balances: [
    ...
    expect(result.results.find(r => r.employeeId === 'E1')?.status).toBe('SUCCESS');
  ...
  it('Batch Out-of-Order Arrival', async () => {
    ...
    const staleBatch = {
      batchId: 'b3', generatedAt: '2026-04-24T13:00:00Z',
      balances: [{ employeeId: 'E1', locationId: 'L1', balance: 15.0 }]
    };

    await expect(service.processBatchReconciliation(staleBatch)).rejects.toThrow(StaleBatchException);
  });
});