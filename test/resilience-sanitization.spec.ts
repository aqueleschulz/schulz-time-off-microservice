...
import { InvalidDimensionException, DependencyUnavailableException } from '../src/domain/exceptions';
...
describe('Resilience & Sanitization', () => {
  ...
  it('HCM Returns 500 on Deduction (Rollback)', async () => {
    ...
    await expect(service.requestTimeOff(req, 'd1-key')).rejects.toThrow(DependencyUnavailableException);
    expect(await mockRepo.getBalance('E1', 'L1')).toBe(10.0);
    expect(mockRepo.getAuditLogs().some(l => l.type === 'ROLLBACK_AFTER_HCM_FAILURE')).toBeTruthy();
  });

  it('Invalid EmployeeId Format (SQLi Defense)', async () => {
    const req = { employeeId: "'; DROP TABLE employees; --", locationId: 'L1', amount: 2.0 };
    
    await expect(service.requestTimeOff(req, 'e1-key')).rejects.toThrow(InvalidDimensionException);
  ...
  it('Negative Amount and Float Precision Error', async () => {
    ...
    await expect(service.requestTimeOff(reqNegative, 'e2-key')).rejects.toThrow(InvalidDimensionException);
    await expect(service.requestTimeOff(reqFloat, 'e4-key')).rejects.toThrow(InvalidDimensionException);
  });
});